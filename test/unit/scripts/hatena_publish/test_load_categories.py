...
# 準正常系
def test_load_categories_raises_key_error_for_missing_stem(hatena_config: Path) -> None:
    """存在しないstemに対してKeyErrorが発生する"""
    with pytest.raises(KeyError, match="カテゴリ設定が見つかりません"):
        load_categories(hatena_config, "unknown_stem")
