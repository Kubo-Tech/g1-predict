...
- **レコード番号（1始まり）= (曜日オフセット) × 9 + レース番号**
  - 土曜: オフセット 0（レコード番号 = race_no）
  - 日曜: オフセット 1（レコード番号 = 9 + race_no）
  - 曜日オフセット = `date(year, month, day).weekday() - 5`（土曜=0, 日曜=1）
- ファイル内のレコード数 = そのファイルの入力済みレース数（16〜20）
...
- **スロット N（0始まり）= 馬番 N-2 の印**（有効範囲: N=3〜20、馬番1〜18）
- スロット N のバイトオフセット = N × 2
- スロット 0〜2 は常にスペース（未使用）
