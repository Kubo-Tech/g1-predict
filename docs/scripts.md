...
| `## 前日の傾向` | 前日の同競馬場・同芝ダのレース結果（`prev_day_trend`） |
...
対象レースの**前日**に同じ競馬場・同じ芝ダ区分で行われた平地レース（障害を除く）を集計する。

- `### 出目` … 3着以内に入った馬を、人気・枠番・脚質・上がり順位のグループごとに頭数集計した表。
- 各レースごとの `### {競馬場}{R} {条件} {距離}m {頭数}頭` と、3着以内の枠・馬番・人気・4角通過順・上がり3F・上がり順位の表。

前日の傾向を独立記事として出す場合は、生成された `## 前日の傾向` セクションを `前日の傾向.md` へ切り出して使う（例: `public/2026/2026061409030411_宝塚記念/前日の傾向.md`）。
