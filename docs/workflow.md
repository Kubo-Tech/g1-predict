...
`前日の傾向.md` が生成される。前日に同じ競馬場・同じ芝ダで行われた平地レースの出目（人気・枠番・脚質・上がり順位）と、各レースの上位3頭が並ぶ。

- 馬場がどちらに振れているか（内前が残るのか外差しが決まるのか）の所感を書き足す。
- 予想記事から参照するので、**予想記事より先に公開して URL を確定させる**。
...
`予想.md` が生成される。中身は `## ポイント` / `## 関連記事` / `## 印` / `## 見解` / `## 買い目`。

- `## 見解` には、印を付けた馬の過去走コメント（TARGET に書き溜めたもの）が並ぶ。これを土台に読みを書き足す。
- `## 関連記事` は空のまま出力される。傾向分析・前日の傾向の記事リンクを手で貼る。はてなの記事 URL は投稿後に確定するため、**先にそれらを公開してから URL を貼る**。
