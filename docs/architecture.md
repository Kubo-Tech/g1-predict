...
コード（競馬場コード、馬場状態コード）から名称への変換は、どちらのライブラリにも同等の関数があるが、**`keiba-domain` に一本化している**。定義が3箇所に散らばると不整合に気づけないため。ただし表示名がこのリポジトリの記事の書き方と異なる場合（馬場状態の「稍重」「不良」など）は、`keiba-domain` の値を表示名へ変換するマップを g1-predict 側に持つ。
